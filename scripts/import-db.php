...
if (file_exists('exports/' . $filename)) {
    ...
    try {
        ...
        /*
         * In case we need to deactivate plugins.
         * In the below case: Jetpack and W3 total cache
         *
         */

        if (array_search("jetpack/jetpack.php", $active_plugins)) {
            unset($active_plugins[array_search("jetpack/jetpack.php", $active_plugins)]);
        }

        if (array_search("w3-total-cache/w3-total-cache.php", $active_plugins)) {
        ...
        $stmt = $pdo->prepare("UPDATE wp_options SET option_value = replace(option_value, ?, ?) WHERE option_name ='siteurl';");
